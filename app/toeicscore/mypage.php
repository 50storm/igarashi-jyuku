...
try{
	...
	//$toeic_total_page=$TOEIC->getTotalPage(PER_PAGE);
	//d($toeic_total_page);
	
	//if (isset($_GET['page'])){
	//	if(is_numeric($_GET['page']) && $_GET['page'] <= $toeic_total_page ){
	//		$page= (int)$_GET['page'];
	//	}else{
	//		$page=1;
	//	}
	//}else{
	//	$page=1;//クエリパラメータがセットされていなければ、１ページ
	//}
	
	//$from   = PER_PAGE*($page-1);
	//$toeic_rows=$TOEIC->getRows($_SESSION['user_id'], $from, PER_PAGE);
	$toeic_rows=$TOEIC->getRows($_SESSION['user_id']);
	
	//$html_toeic = $TOEIC->showTable($toeic_rows ,  $toeic_total_page, $page );
	$html_toeic = $TOEIC->showTable($toeic_rows);
	//TODO:分析用データを実装中
	//初受験日
	$first_date = $TOEIC->getFristDate($_SESSION['user_id']);
	if( $debug === "on" ){var_dump($first_date);}

	//ラスト受験日
	$last_date = $TOEIC->getLastDate($_SESSION['user_id']);
	if( $debug === "on" ){var_dump($last_date);}

	//受験回数
	$times = $TOEIC->getTimes($_SESSION['user_id']);
	if( $debug === "on" ){var_dump($times);}

	//最高点
	$MaxScores = $TOEIC->getMaxScores($_SESSION['user_id']);
	if( $debug === "on" ){var_dump($MaxScores);}
	
	//平均点
	$AvgScores = $TOEIC->getAvgScores($_SESSION['user_id']);
	if( $debug === "on" ){var_dump($AvgScores);}

	//伸び率
	$rate = $TOEIC->getRate($_SESSION['user_id']);
	if( $debug === "on" ){var_dump($rate);}
	
	$html_analysis_table = $TOEIC->showAnalysisTable($first_date,  $last_date,  $times,  $MaxScores,  $AvgScores,  $rate);
	//d($html_analysis_table);
